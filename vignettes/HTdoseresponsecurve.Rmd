...
1. No negative control wells exist. *Use case example*: data are already normalized.
2. One or more wells are the shared negative control for all other treatments with that sample type on the plate. *Use case example*: All drugs on the plate were dissolved in DMSO, and DMSO-alone wells 
were run on the plate as a control.
3. For each treatment, there is one or more well with a (presumably zero) concentration of that treatment that should be considered a negative control. This case is appropriate if each treatment has its own no treatment wells, labeled with the name of that treatment. *Use case example*: The plate carries Olaparib and Rucaparib treatments, and each has a DMSO control, labeled as "Olaparib" and "Rucaparib" respectively, with drug concentration labeled as 0 for control wells.
...
Note the underscore in **sample_type**. These headers may be in any order, and additional columns may be present. If your text file has the optional column **hour**, data from this column will be loaded into the dataset.
...
So far we have separately specified measurement values (in the *plate* variable) and meta-data values (in the *plate_map* variable). It is common to re-use the same plate map information for data from many timepoints. To analyze the data we will combine the measurements and metadata into a single dataset. 

When combining the plate map and raw data, you must specify **which wells (if any) correspond to negative controls**, also known as vehicle-treated wells. Negative controls are described above in the section **Specifying how to normalize data**; please review that section if you skipped it.
...
```{r plot_ts, fig.height=8, fig.width=8}
...
```{r plot_fit_statistic_AUC, fig.height=5, fig.width=8, warning=FALSE}
ps = plot_fit_statistic( fits[fits$treatment=="drug13",], 
                    statistic = "AUC", 
                    alpha = 0.0025,
                    ylim=c(0, 1.5),
                    main="Area Under the Dose Response Curve, drug 13")
legend(0, 0.3, unique(ps$sample_type), col=unique(ps$color), pch=19, bty="n" )
legend(36, 0.3, c("P>alpha", "P<=alpha"), 
...
plot( fit_DRC( ds, 
     ...
     xlab="nM",
     ylim=c(0, 1.5))
legend(2, 0.4, c("line 1", "line 2"), fill=c("black", "red"), bty="n" )
```
...
legend(0, 0.3, unique(ps$sample_type), fill=unique(ps$color), bty="n" )
legend(36, 0.3, c("P>alpha", "P<=alpha, min>obs_min", "P<=alpha, min<=min_obs"), 
...
the vehicle condition). In one reality the dataset (DS_add), they have merely 
additive effects when combined. In an alternate reality (DS_syn), they have 
evidence for synergistic interactions when combined. The Excess over Bliss 
score is plotted as a heat map using the *plot_color_grid()* function.
...
b_add = synergy_bliss(DS_add, "drug1", "drug2")
b_syn = synergy_bliss(DS_syn, "drug1", "drug2")
layout(matrix(1:2,1,2))
par(mar=c(4,4,3,1))
