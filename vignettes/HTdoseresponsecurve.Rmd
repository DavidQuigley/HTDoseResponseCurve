...
* Data can be loaded in R, from plain text files, or from raw data exported by 
...
1. No negative control wells exist, or the data are already normalized
2. One or more wells are the shared negative control for all other treatments with that sample type on the plate. *Use case example*: All drugs on the plate share a common DMSO control.
3. For each treatment, there is one or more well with a (presumably zero) concentration of that treatment that should be considered a negative control. This case is appropriate if each treatment has its own no treatment wells, labeled with the name of that treatment. *Use case example*: The plate carries Olaparib and Rucaparib treatments, and each has a DMSO control, labeled as "Olaparib 0 nM" or "Rucaparib 0 nM" respectively.
...
These headers may be in any order, and additional columns may be present.
...
*HTDoseResponseCurve* can also read data from 6-, 12-, 24-, 96-, and 384-well plates.  If your data did not originate in a plate format, or if plate information has been discarded, they may be loaded directly as vectors of observations using the same plate identifier for all observations. 
...
legend(0, 100, c("L1 DMSO", "L1 5000 nM", "L2 DMSO","L2 5000 nM"),
       pch=c(19,19,22,22), 
...
layout(matrix(1:2,1,2))
par(mar=c(4,4,3,1))
gi = plot_color_grid(b_add$excess, color_bounds = c(-1,1), 
                main="additive", xlab="nM drug 1", ylab="nM drug 2", 
                plot_values = TRUE)
gs = plot_color_grid(b_syn$excess, color_bounds = c(-1,1), 
