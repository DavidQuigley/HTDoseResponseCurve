...
plot( fit_1, 
      xlab="nM drug", ylab="Normalized confluence", 
...
Note that in the *LL.3()* and *LL.2()* calls, the curves asymptotically approach zero because we specified that the lower asympotote should be zero. The two plots have markedly different $EC_{50}$ estimates from *LL.4()* on the left because the $EC_{50}$ is measured relative to the upper and lower asymptotes for the curve, and the curves have different lower asymptotes.
...
the vehicle condition). Heat maps plotted using the *plot_color_grid()* function 
show the excess over Bliss independence.
...
layout(matrix(1:2,1,2))
gi = plot_color_grid(b_ind$excess, color_bounds = c(-1,1), 
...
We can look at the raw values directly:
